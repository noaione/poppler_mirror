...
NSSSignatureCreation::NSSSignatureCreation(const std::string &certNickname, HashAlgorithm digestAlgTag) : hashContext(HashContext::create(digestAlgTag)), signing_cert(nullptr), estimated_size(CryptoSign::defaultMaxSignatureSize)
{
    NSSSignatureConfiguration::setNSSDir({});
